...
GTEST_REPORT=${GTEST_REPORT_DIR}/volumedriver_test.report.xml

mkdir -p ${GTEST_REPORT_DIR}

./volumedriver_test \
    --gtest_filter=-MTVolume*:*ClusterCacheSerializationTest.device_serialization_1*:*ClusterCacheSerializationTest*ze_big_one*:ClusterCacheMapTest.test1:FawltyTests*:*MetaDataStoreTest.*performance* \
    --disable-logging \
