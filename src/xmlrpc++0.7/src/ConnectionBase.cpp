...
// Static data
const char ConnectionBase::METHODNAME_TAG[] = "<methodName>";
const char ConnectionBase::PARAMS_TAG[] = "<params>";
const char ConnectionBase::PARAMS_ETAG[] = "</params>";
const char ConnectionBase::PARAM_TAG[] = "<param>";
const char ConnectionBase::PARAM_ETAG[] = "</param>";

const std::string ConnectionBase::SYSTEM_MULTICALL = "system.multicall";
const std::string ConnectionBase::METHODNAME = "methodName";
const std::string ConnectionBase::PARAMS = "params";

const std::string ConnectionBase::FAULTCODE = "faultCode";
const std::string ConnectionBase::FAULTSTRING = "faultString";



// The server delegates handling client requests to a serverConnection object.
ConnectionBase::ConnectionBase(xmlrpc::ServerBase* server)
    : _server(server)
{
...
// unsigned
// ConnectionBase::handleEvent(unsigned /*eventType*/)
// {
...
bool
ConnectionBase::readHeader(int fd,
                           bool& keepAlive)
{
  // Read available data
  bool eof;
  if ( ! XmlRpcSocket::nbRead(
                              fd, //this->getfd(),
                              _header, &eof)) {
  ...
  LOG_DEBUG("Read " <<  _header.length() << " bytes");
  ...
  if (lp == 0) {
      LOG_ERROR("No Content-length specified");
  ...
  LOG_DEBUG("specified content length is " <<  _contentLength);
  ...
  if (_header.find("HTTP/1.0") != std::string::npos)
  {
      if (kp == 0 || strncasecmp(kp, "keep-alive", 10) != 0)
          keepAlive = false;           // Default for HTTP 1.0 is to close the connection
  }
  else
  {
...
bool
ConnectionBase::readRequest(int fd)
{
  // If we dont have the entire request yet, read available data
    ...
    if (int(_request.length()) < _contentLength) {
        if (eof) {
            LOG_ERROR("EOF while reading request");
            return false;   // Either way we close the connection
        }
        return true;
    }
    ...
    LOG_DEBUG("read " <<_request.length() << " bytes");
    //XmlRpcUtil::log(5, "ConnectionBase::readRequest:\n%s\n", _request.c_str());
...
bool
ConnectionBase::writeResponse(int fd,
                              bool retval)
{
  ...
  if ( ! XmlRpcSocket::nbWrite( fd, // this->getfd()
                                _response,
                                &_bytesWritten))
  {
  ...
  LOG_DEBUG("wrote " <<  _bytesWritten << " of " << _response.length() << " bytes" );
...
void
ConnectionBase::executeRequest()
{
  XmlRpcValue params, resultValue;
  std::string methodName = parseRequest(params);
  LOG_DEBUG("server calling method '" << methodName << "'");
  ...
  try {
    ...
    if ( ! executeMethod(methodName, params, resultValue) &&
      ...
      LOG_DEBUG("fault " << fault.getMessage());
...
std::string
ConnectionBase::parseRequest(x::XmlRpcValue& params)
{
...
bool
ConnectionBase::executeMethod(const std::string& methodName,
                              x::XmlRpcValue& params,
                              x::XmlRpcValue& result)
{
    x::XmlRpcServerMethod* method = _server->findMethod(methodName);
...
bool
ConnectionBase::executeMulticall(const std::string& methodName,
...
void
ConnectionBase::generateResponse(std::string const& resultXml)
{
...
std::string
ConnectionBase::generateHeader(std::string const& body)
{
  std::string header =
    "HTTP/1.1 200 OK\r\n"
    "Server: ";
...
void
ConnectionBase::generateFaultResponse(std::string const& errorMsg, int errorCode)
{
  const char RESPONSE_1[] =
    "<?xml version=\"1.0\"?>\r\n"
