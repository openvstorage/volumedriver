...
namespace XmlRpc {
  ...
  public:
    ...
    //! Constructors
            XmlRpcValue(ValueStruct& vstruct)
          :_type(TypeStruct)
      {
          _value.asStruct = new ValueStruct(vstruct);
      }
    ...
    XmlRpcValue(const void* value, int nBytes)  : _type(TypeBase64)
    {
      _value.asBinary = new BinaryData((const char*)value, ((const char*)value)+nBytes);
    }
